...
public class Scheduler {
    ...
    void TurnAround(){
        for (Process process : AllProcesses) {
            process.setTurnaround_Time(process.getTurnaround_Time()+process.Waited);
        }
    }
    void Statistics(){
        double wait =0;double turns = 0 ;
        for (Process process : AllProcesses) {
            wait += process.Waited;turns+=process.getTurnaround_Time();
            System.out.println(process.getName() +" : waiting time is "+process.Waited +", TurnAroundTime is "+process.getTurnaround_Time());
        }
    ...
    void Start() {
        ...
        while (true) {
            ...
            while (first.size() != 0) {
                ...
                selected.Waited = (Time - selected.getArrival_time()); //calculate the waiting time
                ...
                System.out.println(selected.getName() + " started, waited till now  " + selected.Waited);
            ...
            while (second.size() != 0) {
                ...
                selected.Waited += (Time - selected.Completion); //calculate the waiting time
